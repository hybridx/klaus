...
def _is_complex(text: str, threshold: int = 2) -> bool:
    ...
    if len(sentences) >= threshold:
        return True
    multi_markers = ["then", "and also", "after that", "additionally", "also", "next"]
    lower = text.lower()
...
async def _handle_orchestrated_chat(
    ...
    async for event in state.agent.orchestrate(
        ...
        elif etype == "plan.consolidated":
            await state.event_bus.send_to_session(
                session_id, EventType.PLAN_CONSOLIDATED, {"chat_id": chat_id},
            )
        elif etype == "token":
            full_response += event.get("content", "")
            await state.event_bus.send_to_session(
                session_id, EventType.CHAT_TOKEN,
